	...
	def run(self): 
		...
		while enFct == True:
			...
			if cmde == "QUIT" :
				...
				client = mots[1].upper().rstrip()
				emetteur = str(self.ip)+":"+str(self.port)
				self.dispatch_msg_oneclient(client,mots,emetteur)
			else:
				sock.send(" ?? \n")
	...
	def dispatch_msg(self,mots,sender):
		...
		for c in self.listeClient:
			if c != self:
				...
				c.prompt()
	def dispatch_msg_oneclient(self,client,mots,sender):
		l = len(mots)-1
		msg = ""
		for i in range(l):
			msg = msg+" "+mots[i+1]
